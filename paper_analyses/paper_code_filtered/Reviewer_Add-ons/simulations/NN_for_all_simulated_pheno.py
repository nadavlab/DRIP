...
def clean_x_chunk(X, feature_start=None, drop_cols=("FID", "IID")):
    """
    Technical adaptation only:
    - optionally keep columns from feature_start onward, to mimic old X.iloc[:, 170:].
    - otherwise drop ID columns and use all remaining features.
    """
    X = X.copy()

    if feature_start is not None:
        X = X.iloc[:, feature_start:].copy()
    else:
...
def load_test_y(y_test_file, pheno_name):
    """
    Y_test_all.pkl is expected to be one dataframe.
    If it is accidentally chunked, this also supports that.
    """
    ...
    if isinstance(obj, pd.DataFrame):
        ...
        y_df = pd.concat(chunks, ignore_index=True)

    if pheno_name not in y_df.columns:
        raise ValueError(f"Phenotype '{pheno_name}' not found in {y_test_file}")

    return y_df[pheno_name].astype(int).to_numpy()
...
def main():
    ...
    parser.add_argument("--matched-root", default="/path/to/your/project//simulations/matched_data", help="Root directory containing rep*/X_train_match_to_pheno.pkl etc. PCA-only matched files.")
