...
def prepare_x(
    ...
    """
    Keep all non-ID features, including covariates and UKB PCs.

    From our created chromosome-specific PCs:
        keep only chromosome 1 PCs
        remove chromosome 2-22 PCs

    Expected created PC names:
        0_chr1, 1_chr1, ...
        0_chr2, 1_chr2, ...
    """
    X = X.copy()
    ...
    for col in X.columns:
        ...
        # Identify our chromosome-specific PCs by the "_chr<number>" suffix
        if "_chr" in col_str:
            suffix = col_str.rsplit("_chr", 1)[-1]

            if suffix.isdigit():
                chromosome = int(suffix)

                # Keep chr1 PCs and remove chr2-chr22 PCs
                if chromosome != 1:
    ...
    print(
        f"Feature selection: kept {X.shape[1]} features; "
        f"removed {len(cols_to_remove)} created PCs from chromosomes 2-22.",
        flush=True,
    )

    chr1_pc_cols = [
        c for c in X.columns
        if str(c).endswith("_chr1")
    ]

    print(
        f"Created chromosome 1 PCs retained: {len(chr1_pc_cols)}",
        flush=True,
    )

    if len(chr1_pc_cols) == 0:
        raise ValueError(
            "No chromosome 1 created PCs were detected. "
            "Expected names such as 0_chr1, 1_chr1, 2_chr1."
        )
