...
def LogisticRegression_incremental(x_train_chunks_file, y_train_chunks_file, X_val, y_val, pheno_name, output_path,epoch_from,epoch_to,model):
    ...
    all_columns = None  # will store feature order from first batch
            ...
            with open(y_train_chunks_file, 'rb') as file_handle2:
                ...
                while True:
                    try:
                        ...
                        # drop cov columns to stay with genetic only
                        ...
                        if all_columns is None:
                            all_columns = X_batch.columns.tolist()
                        else:
                            # Ensure column alignment with training structure
                            X_batch = X_batch.reindex(columns=all_columns, fill_value=0)
...
output_path_pre= "/path/to/your/project/LR_with_SNP_selection/"+ pheno_name+"/rep"+rep+"/lr_1e-5"
output_path = "/path/to/your/project/LR_with_SNP_selection/"+ pheno_name+"/rep"+rep+"/lr_1e-5_20_40_epoches_corrected"
...
##########################################predict only or continue training trained model######################################
...
#with open(os.path.join(output_path, "LogisticRegression_model.pkl"), 'wb') as f:
...
metrics_file = os.path.join("/path/to/your/project/classical_lr_model_with_snp_selection/metrics_log_genetic_only.csv")
